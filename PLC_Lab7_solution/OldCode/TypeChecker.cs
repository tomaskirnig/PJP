...
namespace PLC_Lab7
{
    public class TypeChecker
    {
        ...
        // Current target type for operations
        ...
        public object GetVariable(string name, IToken token = null)
        {
            ...
            DataType declaredType = variableTypes[name];

            // Ensure variable consistency with its declared type
            switch (declaredType)
            {
                case DataType.Int:
                    if (!(value is int))
                    {
                        ...
                        {
                            string positionInfo = token != null ? $"{token.Line}:{token.Column} - " : "";
                            throw new Exception($"{positionInfo}Failed to convert value '{value}' to type 'int' for variable '{name}'.");
                        }
                    }
                    break;
                case DataType.Float:
                    if (!(value is float))
                    {
                        ...
                        {
                            string positionInfo = token != null ? $"{token.Line}:{token.Column} - " : "";
                            throw new Exception($"{positionInfo}Failed to convert value '{value}' to type 'float' for variable '{name}'.");
                        }
                    }
                    break;
                case DataType.Bool:
                    if (!(value is bool))
                    {
                        ...
                        {
                            string positionInfo = token != null ? $"{token.Line}:{token.Column} - " : "";
                            throw new Exception($"{positionInfo}Failed to convert value '{value}' to type 'bool' for variable '{name}'.");
                        }
                    }
                    break;
            }
